...
import {
  ...
  MapPin, AlertCircle, MessageSquare, Upload, Edit, Check, Globe, CreditCard, GripVertical, RefreshCw, AlertTriangle, Info, Scale, FolderPlus, XCircle, MicOff, Square, Play, UserPlus, ArrowLeft, Smartphone, Calendar, FileText
} from 'lucide-react';
import {
...
const App: React.FC = () => {
  ...
  const handleSaveTranscript = async () => {
    if (!viewingCand) return;
    try {
      await updateDoc(doc(db, "candidates", viewingCand.id), {
        transcript: tempTranscript
      });
      setViewingCand({ ...viewingCand, transcript: tempTranscript });
      setIsEditingTranscript(false);
      refreshData();
      showAlert("Transkrip berhasil diperbarui!", "success");
    } catch (e) {
      showAlert("Gagal menyimpan transkrip.", "error");
    }
  };
  ...
  const handleEvaluate = async (reEvaluateId?: string) => {
    ...
    try {
      ...
      // Gunakan tempTranscript jika sedang diedit di modal, atau transcript yang ada di object cand
      const transcriptToUse = reEvaluateId ? (tempTranscript || cand.transcript || '') : interviewTranscript;
      ...
      console.error("Evaluasi Error:", e);
      showAlert("Evaluasi gagal. Cek console untuk detail.", "error");
    }
              ...
              {filtered.filter(c => c.status === 'Interview' || c.status === 'Draft').map(c => (
                <div key={c.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-violet-200/40 transition-all flex flex-col group relative overflow-hidden">
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-50 space-y-3">
                    ...
                    <button onClick={() => { setViewingCand(c); setTempTranscript(c.transcript || ''); setIsEditingTranscript(false); }} className="w-full py-3 text-slate-400 font-black text-[10px] uppercase hover:bg-slate-50 hover:text-slate-800 rounded-2xl transition-all tracking-widest">Detail Profile</button>
                  </div>
                </div>
              ))}
            </div>
                    ...
                    {filtered.map(c => (
                      <tr key={c.id} className={`hover:bg-slate-50/50 transition-all group ${selectedForReport.includes(c.id) ? 'bg-violet-50/30' : ''}`}>
                        ...
                        <td className="py-5 px-6">
                          <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${c.status === 'LULUS' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                        ...
                        <td className="py-5 px-8 text-right flex justify-end gap-1">
                          <button onClick={() => { setViewingCand(c); setTempTranscript(c.transcript || ''); setIsEditingTranscript(false); }} className="p-2.5 text-violet-600 hover:bg-violet-50 rounded-xl transition-all"><ChevronRight className="w-5 h-5" /></button>
                          <button onClick={() => {
                            setEditId(c.id);
                            setIsEditing(true);
                            setForm({
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-6xl h-full max-h-[92vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              {/* MAIN CONTENT - Analisis & Tabel & Transkrip */}
              <div className="flex-1 p-8 overflow-y-auto space-y-8">
                ...
                <div className="space-y-4 pt-6 border-t border-slate-100">
                  <div className="flex justify-between items-center">
                    <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-violet-500" /> Transkrip Wawancara</h4>
                    {!isEditingTranscript ? (
                      <button onClick={() => { setIsEditingTranscript(true); setTempTranscript(viewingCand.transcript || ''); }} className="text-[8px] font-black text-violet-600 uppercase tracking-widest flex items-center gap-1 hover:underline">
                        <Edit className="w-3 h-3" /> Edit Transkrip
                      </button>
                    ) : (
                      <div className="flex gap-2">
                        <button onClick={() => setIsEditingTranscript(false)} className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-3 py-1 hover:bg-slate-100 rounded-lg">Batal</button>
                        <button onClick={handleSaveTranscript} className="text-[8px] font-black text-white bg-violet-600 uppercase tracking-widest px-3 py-1 rounded-lg shadow-sm">Simpan Transkrip</button>
                      </div>
                    )}
                  </div>

                  <div className={`p-6 rounded-2xl border-2 transition-all ${isEditingTranscript ? 'bg-white border-violet-100 shadow-inner' : 'bg-slate-50 border-transparent'}`}>
                    {isEditingTranscript ? (
                      <textarea
                        className="w-full h-48 bg-transparent text-[11px] font-medium text-slate-700 leading-relaxed outline-none resize-none"
                        value={tempTranscript}
                        onChange={e => setTempTranscript(e.target.value)}
                        placeholder="Edit transkrip di sini..."
                      />
                    ) : (
                      <div className="text-[11px] font-medium text-slate-500 leading-relaxed max-h-48 overflow-y-auto italic">
                        {viewingCand.transcript ? `"${viewingCand.transcript}"` : "(Tidak ada transkrip tersedia)"}
                      </div>
                    )}
                  </div>
                  {isEditingTranscript && (
                    <p className="text-[8px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" /> Perhatian: Klik 'Kalkulasi Ulang' setelah menyimpan transkrip untuk memperbarui skor AI.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="px-8 py-5 border-t bg-slate-50 flex justify-between items-center">
              <button onClick={() => handleEvaluate(viewingCand.id)} disabled={isEvaluating || isEditingTranscript} className={`px-5 py-2.5 bg-white border border-slate-200 text-amber-600 rounded-xl font-black uppercase text-[9px] tracking-widest flex items-center gap-2 transition-all ${isEditingTranscript ? 'opacity-50 cursor-not-allowed' : 'hover:bg-amber-50'}`}>
                {isEvaluating ? <Loader2 className="animate-spin w-3.5 h-3.5" /> : <RefreshCw className="w-3.5 h-3.5" />} {isEvaluating ? 'Mengevaluasi...' : 'Kalkulasi Ulang AI'}
              </button>

...
// General Questions: 5 questions -> 20% each default
const GENERAL_QUESTIONS: QuestionTemplate[] = [
  { id: 'g1', category: 'General', question: 'Silakan perkenalkan diri Anda secara lengkap (Nama, Asal/Domisili, dan Latar Belakang Keluarga/Pendidikan)?', idealAnswer: 'Menjelaskan identitas diri dengan jelas, sopan, dan percaya diri.', weight: 20 },
  { id: 'g2', category: 'General', question: 'Apa yang Anda ketahui tentang perusahaan kami, dan dari mana Anda mendapatkan informasi tersebut?', idealAnswer: 'Menunjukkan riset mendalam tentang produk, visi, atau budaya perusahaan.', weight: 20 },
  { id: 'g3', category: 'General', question: 'Sebutkan pengalaman kerja paling relevan yang pernah Anda miliki dengan posisi ini?', idealAnswer: 'Memberikan contoh konkret pencapaian atau tugas yang sesuai dengan job desk.', weight: 20 },
  { id: 'g4', category: 'General', question: 'Apa kelebihan terbesar Anda yang bisa berkontribusi langsung, dan apa satu kekurangan yang sedang Anda perbaiki?', idealAnswer: 'Jujur mengenai kelemahan (dengan solusi) dan percaya diri mengenai kelebihan.', weight: 20 },
  { id: 'g5', category: 'General', question: 'Berapa ekspektasi gaji Anda dan kapan Anda siap mulai bekerja?', idealAnswer: 'Realistis sesuai standar pasar dan menunjukkan ketersediaan yang jelas.', weight: 20 }
];

// Helper to add weights to technical (Assuming 10 questions -> 10% each default)
const withWeights = (questions: Omit<QuestionTemplate, 'weight'>[]): QuestionTemplate[] => {
  const weightPerQ = Math.floor(100 / questions.length);
  const remainder = 100 % questions.length;
  
  return questions.map((q, i) => ({
    ...q,
    weight: i === questions.length - 1 ? weightPerQ + remainder : weightPerQ // Add remainder to last question
  }));
};
